...
docs/The_XSLT/JavaScript_Interface_in_Gecko. In each browser’s case, after the transformation, the resulting XHTML markup is
...
• XSL node\_\-\_set function name retrieves the current node’s element name.
...
**14.10** (_Sorting XSLT Modification)_ Modify Fig. 14.23 (sorting.xsl) to sort by the number of pages rather than by chapter number. Save the modified document as sorting_byPage.xsl.

**14.11** Modify Fig. 14.38 to use sorting.xml (Fig. 14.22), sorting.xsl (Fig. 14.23) and sorting_byPage.xsl (from Exercise 14.10). Display the result of transforming sorting.xml using each style sheet. \[_Hint:_ Remove the xml:stylesheet processing instruction from line 2 of sort- ing.xml before attempting to transform the file programmatically.\]
